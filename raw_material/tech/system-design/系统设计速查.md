# System Design Quick Reference — 30 Cases

> Source: Growing - 个人成长管理系统.pdf
> 30 cases. One section per system.
...
| 30  | [[#30. Design SLO/SLI Management Platform]] |
...
## 30. Design SLO/SLI Management Platform

**Requirements**
- Users define SLOs (target + time window) linked to SLIs (data source + query)
- System continuously computes Error Budget (consumed + remaining) per SLO
- Users define Burn Rate Alert rules; system fires when threshold exceeded
- Dashboard: SLO health, Error Budget trend, alert history

**NFR**
- SLI data latency: **< 1 min** (metric produced → Error Budget updated)
- Burn Rate Alert latency: **< 5 min** (threshold breach → alert fired)
- Scale: thousands of SLOs, each with multiple SLIs
- Availability: 99.9%+; alert path high reliability
- Audit: all budget changes and alert history traceable

**Core Math (must know)**
```
Error Budget  = (1 - SLO target) × window
               e.g. SLO=99.9%, 30d → 0.1% × 30d = 43.2 min allowed downtime

Burn Rate     = actual_error_rate / (1 - SLO target)
               e.g. error_rate=1.4%, target=99.9% → 1.4%/0.1% = 14×

Time to exhaustion = window / burn_rate
               burn_rate=14 → 30d/14 = ~2 days until budget gone
```

**Entities:** SLO (target, window, sli_ids), SLI (type, data_source, PromQL query), SLI DataPoint (TSDB: timestamp, good, total), ErrorBudget (consumed %, remaining %), BurnRateSnapshot (per window: 1h/6h/3d), AlertRule (short_window, long_window, threshold, severity), AlertEvent (append-only)

**Components**
- SLI Collector: Pull from Prometheus/Datadog every minute; unified `(timestamp, good, total)` format
- TSDB: Store raw SLI data points (minute-level, 30+ days)
- Budget Calculator: Incremental sliding window — Circular Buffer avoids full 30-day rescan each minute; O(1) update
- Burn Rate Engine: Compute burn rate for multiple windows (1h, 6h, 3d) in parallel
- Alert Evaluator: Multi-window two-condition check (short AND long window both above threshold)
- Notification Service: Dedup per alert rule; fire on state transitions only (firing/resolved)
- Recording Rules (Prometheus): Pre-aggregate expensive queries server-side; Collector only reads cheap results

**Google Multi-Window Alert Thresholds (memorize):**

| Level | Short Window | Long Window | Burn Rate | Meaning | Action |
|-------|-------------|-------------|-----------|---------|--------|
| P1 Critical | 1h | 5h | **14×** | Budget gone in ~2 days | PagerDuty immediately |
| P2 Warning | 6h | 1d | **6×** | Budget gone in ~5 days | Slack, business hours |
| P3 Info | 3d | — | **3×** | Budget gone in ~10 days | Ticket, schedule fix |

**Why two-window?** Short window = "is it still happening NOW?"; Long window = "filters out brief spikes". Both must exceed threshold to fire → reduces alert fatigue.

**API**
```
POST /slos                          → create SLO
POST /slis                          → define SLI (data_source + good_query + total_query)
GET  /slos/{id}/budget              → current Error Budget state
GET  /slos/{id}/burn-rate           → current burn rate per window
POST /slos/{id}/alert-rules         → create burn rate alert rule
GET  /slos/{id}/history             → Error Budget trend (time series)
```

**Key Design Decisions**
- Rolling window not calendar: budget doesn't reset on the 1st of the month; pressure shouldn't spike at month-end
- Incremental Circular Buffer: store 43,200 (good, total) pairs per SLI; O(1) update vs O(N) full rescan
- Multi-window alert (Google SRE Book): P1 at 14× burn rate with 1h+5h windows; prevents false alerts from brief spikes
- Composite SLO: multiple SLIs combined via AND (strictest) or weighted average; shared Error Budget
- Pull + Recording Rules: avoid overloading Prometheus; pre-aggregate expensive queries at source

```mermaid
flowchart LR
    Prom[Prometheus\nDatadog] -->|pull every 1min| Collector[SLI Collector]
    Collector --> TSDB[(TSDB)]
    TSDB --> Calc[Budget Calculator\nIncremental window]
    Calc --> Budget[(ErrorBudget\nPostgres)]
    Budget --> Burn[Burn Rate Engine\n1h / 6h / 3d]
    Burn --> Alert[Alert Evaluator\nshort AND long window]
    Alert --> Notify[Notification\nService]
    SLOConfig[(SLO Config\nPostgres)] --> Calc & Burn & Alert
```

> Full deep dive: [[raw_material/tech/system-design/sd-slo-platform]]
